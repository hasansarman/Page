<div class="box-body">
    <div class="box-body">
        <div class='form-group{{ $errors->has("TITLE") ? ' has-error' : '' }}'>
            ...
            {!! Form::text("TITLE", old("TITLE"), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('page::pages.form.title')]) !!}
            {!! $errors->first("TITLE", '<span class="help-block">:message</span>') !!}
        </div>
        <div class='form-group{{ $errors->has("SLUG") ? ' has-error' : '' }}'>
            {!! Form::label("SLUG", trans('page::pages.form.slug')) !!}
            {!! Form::text("SLUG", old("SLUG"), ['class' => 'form-control slug', 'data-slug' => 'target', 'placeholder' => trans('page::pages.form.slug')]) !!}
            {!! $errors->first("SLUG", '<span class="help-block">:message</span>') !!}
        </div>
         <script src="/assets/akrep_assets/js/vendor/ckeditor/ckeditor.js"></script>
        <h3 for="aciklama">Bodyı</h3>
            ...
            <div style="height: 0px;" id="collapseTwo" class="panel-collapse collapse">
                <div class="box-body">
                    <div class='form-group{{ $errors->has("META_TITLE") ? ' has-error' : '' }}'>
                        ...
                        {!! Form::text("META_TITLE", old("$lang.meta_title"), ['class' => "form-control", 'placeholder' => trans('page::pages.form.meta_title')]) !!}
                    ...
                    <div class='form-group{{ $errors->has("META_DESCRIPTION") ? ' has-error' : '' }}'>
                        ...
                        <textarea class="form-control" name="META_DESCRIPTION" rows="10" cols="80">{{ old("$lang.META_DESCRIPTION") }}</textarea>
            ...
            <div style="height: 0px;" id="collapseFacebook" class="panel-collapse collapse">
                <div class="box-body">
                    <div class='form-group{{ $errors->has("OG_TITLE") ? ' has-error' : '' }}'>
                        ...
                        {!! Form::text("OG_TITLE", old("OG_TITLE"), ['class' => "form-control", 'placeholder' => trans('page::pages.form.og_title')]) !!}
                    ...
                    <div class='form-group{{ $errors->has("OG_DESCRIPTION") ? ' has-error' : '' }}'>
                        ...
                        <textarea class="form-control" name="OG_DESCRIPTION" rows="10" cols="80">{{ old("$lang.OG_DESCRIPTION") }}</textarea>
                    ...
                    <div class="form-group{{ $errors->has("OG_TYPE") ? ' has-error' : '' }}">
                        ...
                        <select class="form-control" name="OG_TYPE">
                            <option value="website" {{ old("OG_TYPE") == 'website' ? 'selected' : '' }}>{{ trans('page::pages.facebook-types.website') }}</option>
                            <option value="product" {{ old("OG_TYPE") == 'product' ? 'selected' : '' }}>{{ trans('page::pages.facebook-types.product') }}</option>
                            <option value="article" {{ old("OG_TYPE") == 'article' ? 'selected' : '' }}>{{ trans('page::pages.facebook-types.article') }}</option>
                        </select>
                    </div>
                </div>
            </div>
